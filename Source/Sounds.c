...
void axFrameCallback()
{
	...
	if (soundSel == STRTBKGND)
	{
		...
		voiceOffsets.endOffset = (694774 - 1);					// loop end at end of data (array size minus 1 see sound data header file).
		...
		voiceOffsets.data = (unsigned char*)BackgroundWave;		// pointer to the actual sound data.
	...
	if (soundSel == START)
	{
		...
		voiceOffsets.loopingEnabled = 1;						// loop enabled for start screen music to repeat until game started.
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (401234 - 1);					// loop end at end of data (array size minus 1)
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)StartWave;
	...
	if (soundSel == INTRO)
	{
		...
		voiceOffsets.dataType = 25;
		voiceOffsets.loopingEnabled = 0;					// Sound effects only play once (which will automatically stop the start music).
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (241152 - 1);
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)IntroWave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);
	...
	if (soundSel == JUMP)
	{
		...
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (43508 - 1);
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)JumpWave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);
	...
	if (soundSel == LEVEL2)
	{
		...
		voiceOffsets.loopingEnabled = 0;
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (137984 - 1);
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)Level2Wave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);
	...
	if (soundSel == LEVEL4)
	{
		...
		voiceOffsets.loopingEnabled = 0;
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (339712 - 1);
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)Level4Wave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);
	...
	if (soundSel == OVER)
	{
		...
		voiceOffsets.loopingEnabled = 0;
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (163840 - 1);
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)GameOverWave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);
...
// This must use the function difinitions and data structures in SoundData.h NOT sndcore2/voice.h. I spent several painful 
// days trying to get sound to work using voice.h.
void setupSound()
{
